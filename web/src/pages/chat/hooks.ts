...
export const useEditDialog = () => {
  ...
  const { handleClickDialog } = useClickDialogCard(); // 添加这一行
  ...
  const onDialogEditOk = useCallback(
    async (dialog: IDialog) => {
      ...
      if (ret?.code === 0) {
        hideModal();
        // 如果是新创建的助理（没有dialog_id），自动选中它
        if (!dialog.dialog_id && ret?.data?.id) {
          // 等待助理列表刷新后再选中新创建的助理
          setTimeout(() => {
            handleClickDialog(ret.data.id);
          }, 100);
        }
      }
    },
    [submitDialog, hideModal, handleClickDialog],
  );
  ...
  const sendMessage = useCallback(
    async ({
      ...
      const userCustomPrompt =
        currentDialog?.prompt_config?.user_custom_prompt || '';
      const systemPrompt = currentDialog?.prompt_config?.system || '';

      // 如果系统提示词不以隐藏提示词开头，则需要拼接
      let finalSystemPrompt = systemPrompt;
      if (!systemPrompt.startsWith(HIDDEN_SYSTEM_PROMPT)) {
        finalSystemPrompt = HIDDEN_SYSTEM_PROMPT + userCustomPrompt;
      }

      // 处理消息数组，确保系统指令位于最前面
      let finalMessageArray = originalMessageArray;
      ...
      if (finalSystemPrompt.trim()) {
        ...
        const systemMessageIndex = finalMessageArray.findIndex(
          (msg) => msg.role === 'system',
        );

        if (systemMessageIndex !== -1) {
          finalMessageArray[systemMessageIndex] = systemMessage;
        } else {
          finalMessageArray = [systemMessage, ...finalMessageArray];
        }
      }
    ...
    [
      ...
      send,
      controller,
      currentDialog, // 添加currentDialog作为依赖项
    ],
  );
